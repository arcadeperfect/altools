...
nuke.menu('Nuke').addCommand('harding/Mask', hardingMask, 'ctrl+shift+i' )
nuke.menu('Nuke').addCommand('harding/Stencil', hardingStencil, 'ctrl+shift+o' )
...
# delete reads
# delete the sequences referenced by read / write nodes DANGER!
...
# makePaths
# create missing paths for write nodes

def makePaths():

    for i in nuke.selectedNodes("Write"):
        
        file = os.path.dirname(i['file'].getValue())
        if not os.path.isdir(file):
            os.makedirs(file)

# cearCam
# clear unwanted keyframes
...
# setLabels
# set handy TCL labels

def setLabels():
    
    for n in nuke.allNodes('FrameRange'):
        if len(n['label'].getValue()) == 0:
            n['label'].setValue('[value this.first_frame] - [value this.last_frame]')
    
    for n in nuke.allNodes('TimeOffset'):
        if len(n['label'].getValue()) == 0:
            n['label'].setValue('[value time_offset]')


def setWriteRange():
    for n in nuke.selectedNodes("Write"):
        n['use_limit'].setValue(1)
        n['first'].setValue(n.frameRange().first())
        n['last'].setValue(n.frameRange().last())
